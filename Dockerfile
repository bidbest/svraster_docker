...
RUN conda init bash && exec bash && conda activate svraster

RUN conda install -n svraster -y -c "nvidia/label/cuda-12.4.0" cuda-toolkit
...
RUN conda run -n svraster pip install -r requirements.txt

COPY ./cuda/ /tmp/cuda
RUN conda run -n svraster pip install -e cuda/
